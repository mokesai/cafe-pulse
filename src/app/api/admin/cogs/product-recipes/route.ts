...
export async function POST(request: NextRequest) {
  ...
  const yieldQty = parseNumber(body.yield_qty) ?? 1
  const yieldUnit = normalizeText(body.yield_unit) || 'each'
  const notes = typeof body.notes === 'string' ? body.notes : null

  if (!productId || !effectiveFrom) {
    return NextResponse.json({ error: 'product_id and effective_from are required' }, { status: 400 })
  }
