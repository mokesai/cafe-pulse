...
export async function GET(
  ...
  const lines = [
    ...
    '# MOK-49/50 (phase 8) — Wayland kiosk stack. sway is the compositor,',
    '# seatd is the seat manager (needed for DRM/input under Wayland on Pi',
    '# OS Lite Bookworm), and foot is the break-glass terminal that the',
    '# sway config binds to Mod4+Return for on-Pi debugging.',
    'echo "[1/5] Installing packages..."',
    'sudo apt-get update -qq',
    'sudo apt-get install -y -qq chromium sway seatd libseat1 foot jq > /dev/null 2>&1 || true',
    '# Pi user needs membership in `video`, `render`, and `seat` to open',
    '# DRM nodes under seatd. `video` is the default; the others may not be.',
    "sudo usermod -a -G video,render,seat pi 2>/dev/null || true",
    '# Ensure seatd is up so sway can claim a seat at boot.',
    ...
    '# MOK-49/50 (phase 8) — sway as a systemd user service with linger',
    "# enabled so the kiosk starts at boot without a console login. Replaces",
    '# the v2-era `.bash_profile` + startx dance entirely.',
    'echo "[4/5] Configuring auto-start..."',
    '',
    '# Install the sway compositor config + systemd user unit.',
    ...
    '',
    '# Defensive cleanup of any prior v2-style autostart (no-op on a fresh',
    "# flash; matters when the operator re-runs setup on a Pi that already",
    '# had the X11 path installed).',
    'sed -i \'/kds-kiosk.sh/d\' "$HOME/.bash_profile" 2>/dev/null || true',
    'sudo rm -f /etc/X11/xorg.conf.d/10-blanking.conf',
    '',
    '# Enable lingering so the pi-user systemd instance starts at boot.',
    '# Replaces the raspi-config console-autologin prerequisite.',
    'sudo loginctl enable-linger pi',
    '',
    '# Reload + enable our unit. `--now` would also start it, but we hold',
    "# off so the operator sees a clean reboot path rather than sway",
    '# trying to take over the SSH session.',
    'systemctl --user daemon-reload',
    'systemctl --user enable kds-kiosk.service',
    '',
    'echo "  Done: Auto-start configured (sway + systemd-user + linger)"',
    '',
    '# Step 5: Done',
