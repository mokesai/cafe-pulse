...
#
# This script will:
#   1. Install required packages (Chromium, X server, utilities)
#   2. Register this device with Cafe Pulse
#   3. Create kiosk launch scripts
...
HOME_DIR="$HOME"
CONFIG_FILE="$HOME_DIR/kds-config.json"
...
sudo apt-get install -y -qq chromium-browser xserver-xorg xinit x11-xserver-utils unclutter jq > /dev/null 2>&1
...
# Download kds-register.sh
curl -sL "${origin}/api/kds/kiosk-script?type=register" > "\\$HOME_DIR/kds-register.sh"
chmod +x "\\$HOME_DIR/kds-register.sh"

# Download kds-kiosk.sh
curl -sL "${origin}/api/kds/kiosk-script?type=kiosk" > "\\$HOME_DIR/kds-kiosk.sh"
chmod +x "\\$HOME_DIR/kds-kiosk.sh"

# Write API base to config
jq --arg api "\\$API_BASE" '. + {api_base: \\$api}' "\\$CONFIG_FILE" > "\\${CONFIG_FILE}.tmp" \\
...
if ! grep -q "kds-kiosk.sh" "\\$HOME_DIR/.bash_profile" 2>/dev/null; then
  echo "\\$AUTOSTART" >> "\\$HOME_DIR/.bash_profile"
fi
