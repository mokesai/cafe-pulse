...
**Status:** In flight — T1-T7 complete, mid-T8 manual verification
...
## 2026-05-12 — Path A pivot (mid-flight scope adjustment)

After the operator first saw the rendered editor on 2026-05-11, three new editor capabilities surfaced as desirable:

1. **Variable column heights** — column A might have 1 row-tall box, column B has 2, column C has 3.
...
### T-A — Raise grid max from 12 to 24 (Path A — 2026-05-12)
**Files:**
- `supabase/migrations/<timestamp>_kds_v3_phase_2_grid_max_24.sql` (new, separate from T1)
- `.planning/kds-v3/PHASE-2-ROLLBACK.sql` (extend with the new ALTER undo step)
- `src/app/api/admin/kds-v3/screens/route.ts` (route validation)
- `src/app/api/admin/kds-v3/screens/[id]/route.ts` (route validation)
- `src/components/admin/kds-v3/ScreenForm.tsx` (rows/cols input `max` attribute)

**Scope:**
- New migration that drops + recreates the CHECK constraints on `kds_screens.grid_rows` and `kds_screens.grid_cols` to allow `BETWEEN 1 AND 24` (was 1..12).
  - Postgres can't ALTER an existing CHECK in place; the migration does `ALTER TABLE … DROP CONSTRAINT … ADD CONSTRAINT … CHECK (… BETWEEN 1 AND 24)`.
  - Constraint names are auto-generated by Postgres if T1 didn't name them; the migration looks them up via `information_schema.check_constraints` (or names them explicitly in T1 for future-proofing — T-A picks the auto-name lookup approach to avoid touching T1).
- Route validators (`POST /screens`, `PUT /screens/[id]`): update the `grid_rows < 1 || > 12` checks to `> 24`. Validation error message updated to match.
- UI `ScreenForm.tsx`: change `max={12}` on the rows/cols numeric inputs to `max={24}`.
- Existing T7 integration tests use 4×6 / 2×2 grids — within the new range, still passing.

**Why a separate migration (not editing T1):** T1's migration has already been applied to cafe-pulse-dev. Editing T1 would require us to roll back + re-apply to pick up the change. A new migration is the cleaner forward path AND it's the same pattern that lands on staging/main later. T1 stays the "original schema" record; T-A is the "Path A bump" record. Rollback covers both.

**Acceptance:**
- `npm run db:migrate` applies T-A cleanly to cafe-pulse-dev.
- Inserting a screen with `grid_rows = 20` now succeeds (was rejected previously).
- Inserting a screen with `grid_rows = 25` still raises CHECK violation.
- Route validation rejects body with `grid_rows = 30` at 400 BAD_REQUEST.
- UI's rows/cols numeric input accepts up to 24 (manually verified in T8 step 4 with a 24×24 grid).
- All existing T7 tests still pass.
...
**Procedure:**
1. Apply migrations via `npm run db:migrate` (includes both T1 and T-A).
2. Sign in to `http://bigcafe.localhost:3000` as a bigcafe admin.
3. Navigate to `/admin/kds-v3/screens`.
4. Create screen "Drinks" (4 rows × 6 cols, warm theme). Drag/resize; verify position numbers (1, 2, 3) stable across edits.
5. Save; reload page; confirm state persists with same positions.
6. Create a second screen "Food" with a **higher-resolution grid (e.g. 16 rows × 12 cols)** to exercise the T-A bump. Place boxes that simulate variable column heights (column A: one 16-row box; column B: two 8-row boxes; column C: four 4-row boxes). Confirm the editor handles the higher resolution without layout glitches.
...
9. Direct POST with `grid_rows = 25` — confirm 400 validation error (T-A upper bound).
10. Delete a screen — confirm box rows are gone via Supabase Studio.
...
- T7 needs T3 + T4 + T6.
- **T-A lands after T7, before T8.** T7 tests assert the 12-max contract via business logic, not via the constraint literal; they continue to pass after the bump. T-A's own acceptance is verified during T8.
- T8 + T9 are end-of-phase, after T1-T7 + T-A land.
...
| Grid resolution supports variable column heights / adjustable per-box heights via 24-max | T-A + T8 #6 |
