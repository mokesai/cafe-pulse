...
- **2026-05-31 — systemd-user + linger cannot grant sway a seat.** After config fix, sway ran cleanly from SSH and rendered the awaiting page to both TVs. But the systemd-user service still restart-looped with exit 1 and no logs. Root cause: `loginctl enable-linger` does not establish a PAM-managed login session, so seatd/logind refuse to grant sway a seat. Resolution: replaced systemd-user + linger with **greetd**, a system-level display manager that runs sway under a proper PAM session.
- **2026-05-31 — greetd Bookworm gotchas.** I assumed greetd's packaged unit had `Conflicts=getty@tty1.service`. It does not — the unit has `After=getty@tty7.service` and `Conflicts=getty@tty7.service` (legacy X11 VT semantics). Also, `WantedBy=graphical.target` but Pi OS Lite defaults to `multi-user.target`, so greetd never activates at boot at all. After `sudo systemctl enable greetd.service`, the service was `inactive (dead)` with no journal entries because systemd never started it. Resolution: setup script now also runs `sudo systemctl set-default graphical.target` (so greetd activates) and `sudo systemctl mask getty@tty1.service` (so greetd has VT-1 unchallenged).
- **2026-05-31 — kiosk script dies silently on boot-time network race.** After greetd + graphical.target fix, greetd successfully started sway on vt1 (sway + swaybg visible in `ps`), but the screen stayed blank and no Chromium processes were running. The kiosk log showed the script reaching `"Device <id> registered. Fetching latest config..."` and going no further. Root cause: at boot, greetd → sway → kiosk script fires before WiFi/DHCP completes, so `curl` to the config endpoint returns non-zero (DNS or connect), `set -e` kills the script silently with no further log output. Compounded by Pi OS Bookworm's cloud-init wiping the operator's `/etc/hosts` entry for `bigcafe.local-macbook` on each boot. Resolution: kiosk script now retries the config fetch up to 10 times with 3-second backoff (~30s patience), falls back to the cached `kds-config.json` if the server stays unreachable, and never silently dies on a `curl` exit code. Cloud-init `/etc/hosts` wipe filed as a separate operator-environment ticket.
